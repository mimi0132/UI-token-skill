...
> **Single purpose**: 给现有组件库换皮肤。
> **唯一输出**: `overrides/<lib>-theme-override.css` —— 导入一次,所有现有组件自动变脸。
...
| ✅ Does | ❌ Does NOT |
|---------|-------------|
| 提取设计 token (颜色/字体/间距/圆角/阴影) | 生成新组件 |
| 输出 1 个 override CSS 文件 | 修改用户现有代码 |
| 输出 5 个 token 源文件 | 改 API、props、组件结构 |
| 输出预览页 (用户先看效果) | 假设用户没有基础组件库 |
| 告诉用户怎么 1 行 import 集成 | 强制选某个组件库 |
